...
export const ExerciseListFilter = () => {
    ...
    const [selectedLevel, setSelectedLevel] = useState<string>(paramsLevel ?? "");
    ...
    useEffect(() => {
        const filters = [selectedMuscle, selectedEquipment, selectedCreated, selectedLevel];
        const appliedFilters = filters.filter((filter) => filter !== "");
        filterAmount.current = appliedFilters.length;

        router.push(
            `?muscle=${selectedMuscle}&equipment=${selectedEquipment}&created=${selectedCreated}&level=${selectedLevel}&filter=${searchFilter}`,
            { scroll: false }
        );
    }, [selectedMuscle, selectedEquipment, selectedCreated, searchFilter, selectedLevel, router]);
        ...
        <div className='flex gap-2 flex-wrap'>
            ...
            <DropdownMenu>
                ...
                <DropdownMenuContent align='start' className='w-56'>
                    ...
                    <DropdownMenuSeparator />
                    <DropdownMenuSub>
                        <DropdownMenuSubTrigger>Dificuldade</DropdownMenuSubTrigger>
                        <DropdownMenuSubContent>
                        <DropdownMenuCheckboxItem
                                    onSelect={(event) => event.preventDefault()}
                                    onCheckedChange={() => setSelectedLevel("")}
                                    checked={paramsLevel == null || paramsLevel == ""}
                                >
                                    Todas
                            </DropdownMenuCheckboxItem>
                            <DropdownMenuCheckboxItem
                                    onSelect={(event) => event.preventDefault()}
                                    onCheckedChange={() => setSelectedLevel("Iniciante")}
                                    checked={paramsLevel == "Iniciante"}
                                >
                                    Iniciante
                            </DropdownMenuCheckboxItem>
                            <DropdownMenuCheckboxItem
                                    onSelect={(event) => event.preventDefault()}
                                    onCheckedChange={() => setSelectedLevel("Intermédiario")}
                                    checked={paramsLevel == "Intermédiario"}
                                >
                                    Intermédiario
                            </DropdownMenuCheckboxItem>
                            <DropdownMenuCheckboxItem
                                    onSelect={(event) => event.preventDefault()}
                                    onCheckedChange={() => setSelectedLevel("Avançado")}
                                    checked={paramsLevel == "Avançado"}
                                >
                                    Avançado
                            </DropdownMenuCheckboxItem>
                        </DropdownMenuSubContent>
...
export default function ExerciseListDisplay({ exercises }: { exercises: APIExercise[] }) {
    ...
    const filteredLevel = searchParams.get("level");
    ...
    const exercisesFiltered = exercises?.filter(function (exercise) {
        ...
        const hasMatchingLevel = !filteredLevel || exercise.level.includes(filteredLevel as "Iniciante" | "Avançado" | "Intermediário")
        ...
        return hasMatchingMuscle && hasMatchingEquipment && hasMatchingCreator && hasMatchingLevel;
    });

...
export default function Sidebar({ session }: { session: Session | null }) {
    let [user] = useState<UserProfile | null>(null);
            ...
            >
                <div id='topsection' className='w-full h-auto px-5 pt-5 flex flex-col gap-6'>
                    <div className='flex items-center w-full h-10 gap-4'>
                        ...
                        <div>
                            <h3 className='text-sm font-semibold'>Game Gymn</h3>
                            <h3 className='text-sm text-muted-foreground'>{user?.profile === 'gymOwner'  ? "Dono" : "Membro"}</h3>
                        </div>
                    </div>
                    <SearchCommand />
